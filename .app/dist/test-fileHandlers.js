...
--- [Telemetry Notice: File size is ${(stats.size / (1024 * 1024)).toFixed(1)} MB. Truncated for viewing performance. Open in Desktop App for full file] ---`;
  ...
  static async openGoogleSuiteSession(appType, windowMode = "station_window", targetFilePath, preferredEngine, explicitUrl) {
    ...
    if (!targetUrl && targetFilePath && import_fs.default.existsSync(targetFilePath)) {
      targetFileName = import_path2.default.basename(targetFilePath);
    ...
    try {
      ...
      if (import_fs.default.existsSync(settingsFile)) {
        ...
        if (!explicitUrl && (!targetFilePath || !import_fs.default.existsSync(targetFilePath))) {
...
`;
  ...
  static openKStars(customPath) {
    ...
    if (process.platform === "win32") {
      ...
      candidates.push(
        import_path2.default.join(programFiles, "KStars", "bin", "kstars.exe"),
        import_path2.default.join(programFiles, "kstars", "bin", "kstars.exe"),
        import_path2.default.join(programFilesX86, "KStars", "bin", "kstars.exe"),
        "C:\\KStars\\bin\\kstars.exe",
        import_path2.default.join(programFiles, "KStars", "kstars.exe"),
        import_path2.default.join(localAppData, "Programs", "KStars", "bin", "kstars.exe"),
        import_path2.default.join(localAppData, "Programs", "kstars", "bin", "kstars.exe")
      );
    ...
    for (const cand of candidates) {
      if (cand && import_fs.default.existsSync(cand)) {
        try {
